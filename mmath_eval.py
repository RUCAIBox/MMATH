...
parser.add_argument("--model_name_or_path", type=str, required=True, help="The model name or path to the model.")
...
if 'checkpoint' in model_name_or_path:
    model_name = model_name_or_path.split("/")[-2] + '/' + model_name_or_path.split("/")[-1]
else:
    model_name = model_name_or_path.split("/")[-1]

sampling_params = SamplingParams(temperature=0.6, top_p=0.95, max_tokens=32768, n=4)
llm = LLM(model=model_name_or_path, max_model_len=36000, tensor_parallel_size=args.tensor_parallel_size)
...
def save_results(mmath, lang):
    ...
    with open(f'results/{model_name}/{lang}.json', 'w+', encoding='utf-8') as f:
...
for lang in LANGUAGE:
    with open(f'{args.bench}/{lang}.json', 'r', encoding='utf-8') as f:
